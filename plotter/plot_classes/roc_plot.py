...
class RocPlotBase(PlotBase):
    def plot(self):
        required_params = {
            'n_ratio_panels', 'ylabel', 'xlabel', 'atlas_second_tag', 'figsize', 'y_scale'
        }
        filtered_params = {key: value for key, value in self.config.style.items() if key in required_params}
